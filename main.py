...
def save_user_settings(email, wavespeed_api_key):
...
def save_settings():
    try:
        ...
        save_user_settings(email, wavespeed_api_key)
...
def generate():
    try:
        ...
        model_name = request.form.get("model_name", "flash").strip().lower()
        ...
        if not user_api_key:
            ...
            return jsonify({"error": "Najprv si uloz WaveSpeed API key v Settings."}), 400
